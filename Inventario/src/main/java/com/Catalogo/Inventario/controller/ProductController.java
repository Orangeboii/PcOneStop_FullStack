...
public class ProductController {
    ...
    @Operation(
        summary = "Ver catálogo completo de componentes PC",
        description = "Obtiene el listado completo de componentes de computadora disponibles en PcOneStop. " +
                     ...
                     "Este endpoint requiere autenticación JWT (cualquier usuario autenticado puede acceder). " +
                     "Útil para el PC Builder donde los usuarios necesitan ver todos los componentes disponibles para construir su PC. " +
                     "Los productos incluyen información de stock, precio, categoría y descripción."
    )
    @ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200",
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "401",
            description = "No autenticado: se requiere token JWT válido para ver todos los productos",
            content = @Content(mediaType = "application/json")
        )
    })
    ...
    public ResponseEntity<ApiResponse<Product>> save(HttpServletRequest request) {
        ...
        try {
            ...
            // Validar stock: si es null, establecer en 0; si es negativo, retornar error
            if (product.getStock() == null) {
                product.setStock(0);
                logger.info("Stock no proporcionado, establecido en 0 por defecto");
            } else if (product.getStock() < 0) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse<>(false, 400, "El stock no puede ser negativo", null, 0L));
            }
    ...
    @Operation(
        summary = "Actualizar componente del catálogo",
        description = "Permite a los administradores actualizar la información de un componente existente en el catálogo de PcOneStop. " +
                     "Se pueden actualizar todos los campos: nombre, marca, modelo, categoría, precio, stock, descripción, imagen y campos de oferta. " +
                     "Requiere autenticación JWT con rol ADMIN."
    )
    @ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            ...
            content = @Content(
                mediaType = "application/json",
                examples = @ExampleObject(value = "{\"ok\": true, \"statusCode\": 200, \"message\": \"Producto actualizado\", \"data\": {\"id\": 1, \"name\": \"GeForce RTX 4070\", \"brand\": \"MSI\", \"model\": \"Ventus 3X\", \"category\": \"GPU\", \"price\": 699.99, \"stock\": 10, \"isOnSale\": true, \"discount\": 15}, \"count\": 1}")
            )
        ),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "400",
            ...
            content = @Content(
                mediaType = "application/json",
                examples = @ExampleObject(value = "{\"ok\": false, \"statusCode\": 400, \"message\": \"El precio es obligatorio y debe ser mayor o igual a 0\", \"data\": null, \"count\": 0}")
            )
        ),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "401",
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "403",
            description = "Acceso denegado: se requiere rol ADMIN para actualizar productos",
            content = @Content(mediaType = "application/json")
        ),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "404",
    ...
    @io.swagger.v3.oas.annotations.parameters.RequestBody(
        description = "Datos actualizados del componente PC. Todos los campos son opcionales excepto que deben ser válidos si se proporcionan. " +
                     "Se pueden actualizar: name, brand, model, category, price, stock, description, image, isOnSale, discount, offerStartDate, offerEndDate. " +
                     "También se puede enviar un objeto 'offer' con {discount, startDate, endDate} para configurar ofertas.",
        required = true,
        content = @Content(
            mediaType = "application/json",
            ...
            examples = {
                @ExampleObject(
                    name = "Actualizar producto básico",
                    summary = "Actualizar campos básicos",
                    value = "{\"name\":\"GeForce RTX 4070 Super\",\"price\":799.99,\"stock\":15}"
                ),
                @ExampleObject(
                    name = "Poner producto en oferta",
                    summary = "Actualizar campos de oferta",
                    value = "{\"isOnSale\":true,\"discount\":20,\"offerStartDate\":\"2024-01-01\",\"offerEndDate\":\"2024-12-31\"}"
                ),
                @ExampleObject(
                    name = "Actualizar con objeto offer",
                    summary = "Usando el objeto offer del frontend",
                    value = "{\"offer\":{\"discount\":25,\"startDate\":\"2024-06-01\",\"endDate\":\"2024-06-30\"}}"
                )
            }
        )
    )
    ...
    @SecurityRequirement(name = "bearerAuth")
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Product>> update(@PathVariable Long id, HttpServletRequest request) {
        String jsonBody = null;
        try {
            ...
            try (BufferedReader reader = request.getReader()) {
                jsonBody = reader.lines().collect(Collectors.joining(System.lineSeparator()));
                logger.info("=== JSON BODY RAW (ACTUALIZAR PRODUCTO) ===");
                logger.info("Body recibido: {}", jsonBody);
            } catch (IOException e) {
                logger.error("ERROR al leer el body del request: {}", e.getMessage(), e);
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
            ...
            try {
                updatedProduct = objectMapper.readValue(jsonBody, Product.class);
                logger.info("=== DESERIALIZACIÓN PRODUCTO ===");
                logger.info("Product deserializado: {}", updatedProduct);
                logger.info("isOnSale: {}", updatedProduct.getIsOnSale());
                logger.info("discount: {}", updatedProduct.getDiscount());
                logger.info("offerStartDate: {}", updatedProduct.getOfferStartDate());
                logger.info("offerEndDate: {}", updatedProduct.getOfferEndDate());
            } catch (Exception e) {
                logger.error("ERROR al deserializar JSON: {}", e.getMessage(), e);
                logger.error("JSON que falló: {}", jsonBody);
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
            ...
            if (updatedProduct.getPrice() != null && updatedProduct.getPrice() < 0) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse<>(false, 400, "El precio no puede ser negativo", null, 0L));
            }
            if (updatedProduct.getStock() != null && updatedProduct.getStock() < 0) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse<>(false, 400, "El stock no puede ser negativo", null, 0L));
            }
            if (updatedProduct.getDiscount() != null && (updatedProduct.getDiscount() < 0 || updatedProduct.getDiscount() > 100)) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiResponse<>(false, 400, "El descuento debe estar entre 0 y 100", null, 0L));
            }
            
            // Validar longitud de imagen si está presente
            if (updatedProduct.getImage() != null && updatedProduct.getImage().length() > 65535) {
                logger.warn("Imagen demasiado larga ({} caracteres), truncando a 65535", updatedProduct.getImage().length());
                updatedProduct.setImage(updatedProduct.getImage().substring(0, 65535));
            }
            
            logger.info("Actualizando producto en la base de datos...");
            Product savedProduct = productService.updateProduct(id, updatedProduct);
            logger.info("Producto actualizado con ID: {}", savedProduct.getId());
            logger.info("=== PRODUCTO ACTUALIZADO EXITOSAMENTE - ID: {} ===", savedProduct.getId());
            
            return ResponseEntity.ok(new ApiResponse<>(true, 200, "Producto actualizado", savedProduct, 1L));
            ...
            logger.error("Mensaje: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse<>(false, 500, "Error al actualizar producto: " + e.getMessage(), null, 0L));
        } catch (Exception e) {
            logger.error("=== ERROR AL ACTUALIZAR PRODUCTO ===", e);
            logger.error("Tipo de excepción: {}", e.getClass().getName());
            logger.error("Mensaje: {}", e.getMessage());
