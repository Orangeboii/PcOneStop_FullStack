...
public class ProductServiceTest {
    ...
    public void testReduceStock_ProductoAgotado_LanzaExcepcion() {
        ...
        Product producto = new Product(1L, "GPU", "Asus", "X", "GPU", 100.0, 0, null, null, false, 0, null, null);
        when(productRepository.findById(1L)).thenReturn(Optional.of(producto));

        // CUANDO/ENTONCES: intentar descontar cualquier cantidad lanza excepción
        RuntimeException ex = assertThrows(RuntimeException.class, () -> {
            productService.reduceStock(1L, 1);
        });
        assertTrue(ex.getMessage().contains("Producto agotado") || ex.getMessage().contains("agotado"));
    }
