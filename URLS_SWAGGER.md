...
| `/api/v1/auth/validate` | GET | Validar token JWT y restaurar sesión | ✅ Requerida (token en header) |
...
| `/api/v1/products/{id}` | PUT | Actualizar producto (incluye ofertas) | ✅ | 🔴 ADMIN |
| `/api/v1/products/{id}` | DELETE | Eliminar producto | ✅ | 🔴 ADMIN |
| `/api/v1/products/{id}/stock` | PUT | Reducir stock | ✅ | Requerida |

**Swagger:** http://localhost:8082/swagger-ui.html

**Nota:** El endpoint `GET /api/v1/products` ahora está disponible para cualquier usuario autenticado (no solo ADMIN) para permitir el uso del PC Builder.
...
| `/api/v1/orders` | POST | Crear nuevo pedido (con validación de stock) | ✅ Requerida |
...
**Nota:** El endpoint `POST /api/v1/orders` ahora soporta dos formatos:
- **Formato antiguo:** `{"productIds": "1,2,3"}` (cantidad 1 por producto)
- **Formato nuevo:** `{"items": [{"productId": 1, "quantity": 2}]}` (cantidad variable) ⭐ RECOMENDADO

El backend valida automáticamente el stock y descuenta la cantidad exacta comprada.
