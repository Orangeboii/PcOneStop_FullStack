...
import com.Pedidos.Pagos.service.ProductClientService;
...
public class OrderServiceTest {
    ...
    public void testCreateOrder_EstadoInicialPendiente() {
        // DADO: un pedido sin estado y productos con stock suficiente
        Order pedido = new Order(null, 1L, 500.0, null, "1,2,3", LocalDateTime.now());
        when(orderRepository.save(any(Order.class))).thenAnswer(i -> i.getArguments()[0]);
        
        // Mockear productos con stock suficiente
        ProductDTO product1 = new ProductDTO(1L, "Producto 1", "Brand", "Model", "Category", 100.0, 10, null, null, false, null, null, null);
        ProductDTO product2 = new ProductDTO(2L, "Producto 2", "Brand", "Model", "Category", 100.0, 10, null, null, false, null, null, null);
        ProductDTO product3 = new ProductDTO(3L, "Producto 3", "Brand", "Model", "Category", 100.0, 10, null, null, false, null, null, null);
        
        when(productClientService.getProductById(1L)).thenReturn(product1);
        when(productClientService.getProductById(2L)).thenReturn(product2);
        when(productClientService.getProductById(3L)).thenReturn(product3);
    ...
    public void testCreateOrder_SobreescribeEstadoSiVieneDiferente() {
        // DADO: un pedido con estado "COMPLETADO" (intento de manipulación) y productos con stock
        Order pedido = new Order(null, 1L, 500.0, "COMPLETADO", "1,2", LocalDateTime.now());
        when(orderRepository.save(any(Order.class))).thenAnswer(i -> i.getArguments()[0]);
    ...
    @Test
    public void testCreateOrder_SinProductos() {
        // DADO: un pedido sin productIds (null o vacío)
        Order pedido = new Order(null, 1L, 500.0, null, null, LocalDateTime.now());
        when(orderRepository.save(any(Order.class))).thenAnswer(i -> i.getArguments()[0]);

        // CUANDO: creamos el pedido
        Order resultado = orderService.createOrder(pedido);

        // ENTONCES: el estado es PENDIENTE y no se llama a validación de productos
        assertEquals("PENDIENTE", resultado.getStatus());
        verify(orderRepository).save(pedido);
        verify(productClientService, never()).getProductById(anyLong());
    }
    ...
    public void testFindByUserId_RetornaPedidosDelUsuario() {
        ...
        Order o1 = new Order(1L, 1L, 100.0, "PENDIENTE", "1", LocalDateTime.now());
        Order o2 = new Order(2L, 1L, 200.0, "ENVIADO", "2", LocalDateTime.now());
    ...
    public void testFindById_PedidoExiste() {
        ...
        Order pedido = new Order(1L, 1L, 300.0, "PENDIENTE", "1,2", LocalDateTime.now());
    ...
    public void testUpdateStatus_CambiaEstadoCorrectamente() {
        ...
        Order pedido = new Order(1L, 1L, 100.0, "PENDIENTE", "1", LocalDateTime.now());
        when(orderRepository.findById(1L)).thenReturn(Optional.of(pedido));
        when(orderRepository.save(any(Order.class))).thenAnswer(i -> i.getArguments()[0]);
    ...
    public void testUpdateStatus_CambiarACompletado() {
        ...
        Order pedido = new Order(1L, 1L, 100.0, "ENVIADO", "1", LocalDateTime.now());
        when(orderRepository.findById(1L)).thenReturn(Optional.of(pedido));
        when(orderRepository.save(any(Order.class))).thenAnswer(i -> i.getArguments()[0]);
